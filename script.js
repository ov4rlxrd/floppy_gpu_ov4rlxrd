...
function randomHoleY() {
  // Доля от высоты экрана, чтобы адаптировать отступы
  const safeMarginRatio = 0.12; // 12% от высоты экрана сверху и снизу
  const safeMargin = canvas.height * safeMarginRatio;

  const min = safeMargin + verticalGap / 2;
  const max = canvas.height - safeMargin - verticalGap / 2;
...
function draw() {
  ...
  if (gameState === 'menu') {
    ...
    ctx.font = '24px Arial';
    ctx.textAlign = 'left';
    ctx.fillText('made by ov4rlxrd', 10, 130);
